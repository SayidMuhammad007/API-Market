        ...
        if ($start != null && $finish != null) {
            ...
            foreach ($branches as $branch) {
                ...
                $orders = Order::where('branch_id', $branch->id)
                    ...
                    $benefit_uzs = $orders->sum(function ($order) {
                        return $order->order_price->where('price_id', 1)->sum('price') +
                               $order->baskets->sum(function ($basket) use ($order) {
                                   return $basket->basket_price->where('store.price_id', 1)->sum('store.price_come') * $basket->quantity * $order->dollar;
                               });
                    });
                    
                    $benefit_usd = $orders->sum(function ($order) {
                        return $order->order_price->where('price_id', 2)->sum('price') -
                               $order->baskets->sum(function ($basket) use ($order) {
                                   return $basket->basket_price->where('store.price_id', 2)->sum('store.price_come') * $basket->quantity / $order->dollar;
                               });
                    });
                    
                    $dollarAverage = $orders->pluck('dollar')->avg();
